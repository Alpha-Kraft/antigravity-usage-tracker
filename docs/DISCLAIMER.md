...
## 2. Experimental Features & Data Sensitivity

The "Smart Quota Monitoring" feature relies on strictly **Read-Only** access to internal metrics exposed by the local Antigravity environment (via `localhost`).
*   This functionality is provided "as-is" for **informational purposes only**.
*   It is designed solely to help developers visualize their own local workflow metrics.
*   Internal APIs may change at any time, potentially causing this extension to stop working.
